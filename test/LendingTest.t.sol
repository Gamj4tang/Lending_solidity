...
// import "../src/Lending-drer.sol";
// import "../src/Lending-Sophic.sol";
// import "../src/Lending-nayrang.sol";
// import "../src/ReentrancyAttack.sol";
// import "../src/Lending-jiuo.sol";
// import "../src/Lending-shee.sol";
// import "../src/Lending-huh.sol";
// import "../src/Lending-moon.sol";
...
contract Testx is Test {
    ...
    function testBorrowWithInsufficientCollateralFails() external {
        ...
        {
            (bool success,) = address(lending).call(
                abi.encodeWithSelector(DreamAcademyLending.borrow.selector, address(0x0), 1000 ether)
            );
            assertFalse(success);
    ...
    // attacking test
    /*
    function testWithdrawYieldHacking() external {
        usdc.transfer(user3, 30000000 ether);
        vm.startPrank(user3);
        usdc.approve(address(lending), type(uint256).max);
        lending.deposit(address(usdc), 30000000 ether);
    ...
    // liquidate not myself
    function testNotMyselfLiquidate() external {
        supplyUSDCDepositUser1();
        supplySmallEtherDepositUser2();
        ...
        {
            usdc.approve(address(lending), type(uint256).max);
            lending.liquidate(user2, address(usdc), 500 ether);
            // (bool success,) = address(lending).call(
            //     abi.encodeWithSelector(DreamAcademyLending.liquidate.selector, user3, address(usdc), 500 ether)
            // );
            // assertTrue(success);
        }
        vm.stopPrank();
    ...
    function testOMAttack() external {
        // assume 
        supplyUSDCDepositUser1();
        supplySmallEtherDepositUser2();
